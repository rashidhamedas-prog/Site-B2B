...
- مقصد تلگرام «بررسی دسترسی» گرفت (`getChat/getChatMember`): ادمین‌بودن ربات و اجازهٔ ارسال/ویرایش/حذف ذخیره می‌شود؛ ارسال خودکار فقط به مقصد تأییدشده یا تست می‌رود.
- قالب پست: آلبوم/یک عکس/فقط متن، بولد HTML ایمن، تا ۴ دکمهٔ شیشه‌ای (فقط دامنهٔ ترنم و t.me)، بی‌صدا، ضدفوروارد، متن بالای عکس، پیش‌نمایش لینک؛ پیش‌نمایش شبیه تلگرام با همان رندر سرور.
- قواعد خودکار: حالت خاموش/آزمایشی/زنده، رویدادهای انتخابی، رفتار ناموجودی هر کانال، رفتار حذف محصول، سقف روزانه، فاصلهٔ بین پست، ساعت سکوت تهران.
- سقف ۱۰ محصول زندهٔ canary فقط تا قبل از حالت زنده اعمال می‌شود. خطاهای تلگرام در UI فارسی و قابل فهم شدند.
- گزارش: `docs/reports/2026-09-06-omnichannel-publishing-console-v2.md`
- زنده روی `c7ff21c` (شامل `4b77fc4`): health ۲۰۰، مسیر تأیید مقصد پشت احراز هویت، TTFB هوم عمده از سرور ۱۱۵–۱۸۰ms (بدون تغییر). دیسک VPS پر شده بود (۱۰۰٪) و بیلد اول شکست؛ کش بیلد، ایمیج `broken-20260830` و ایمیج بلااستفادهٔ playwright (۳.۵GB) پاک شد → ۸۳٪. جزئیات و پیشنهاد پاک‌سازی بیشتر در handoff.

## 2026-09-06 — قالب دوم ویترین تک (بوتیک)

- قالب `classic` پیش‌فرض زنده می‌ماند. قالب `boutique` فقط با انتخاب ادمین در «تنظیمات تم ترنم» روی `.ir` می‌آید.
- چیدمان نزدیک به بانی‌مد: هدر تیره، جستجوی قرصی، دو بنر (یک LCP)، ردیف دسته، ریل تخفیف واقعی بدون تایمر دروغین.
- رنگ سبز/طلایی ترنم؛ لوگو و متن بانی‌مد کپی نشد. سایت عمده دست‌نخورده است.

## 2026-09-06 — لینک داخلی سئو هر محصول (جدا برای تکی و عمده)

- گزینه «لینک داخل» به ایجاد/ویرایش محصول در ادمین اضافه شد؛ هر محصول تا ۱۲ لینک per channel، کاملاً جدا برای تکی (.ir) و عمده (.com).
- جدول اختصاصی `product_internal_link` با ستون `channel`؛ اعتبارسنجی هنگام ذخیره (visibility کانال، noindex، self-link، تکرار، طول انکر) و فیلتر هنگام رندر.
- picker ادمین با جستجو/پیشنهاد انکر از focus keyword + دکمه بررسی اعتبار؛ رندر روی PDP با `ItemList` JSON-LD.
- گزارش: `docs/reports/2026-09-06-product-internal-links.md`

## 2026-09-06 — عنوان هوم بدون تکرار برند

- عنوان سند دیگر «پوشاک ترنم» را دوبار نمی‌گذارد؛ پسوند قالب لایوت یک‌بار می‌ماند.
- زنده روی `2723d76`: هوم تکی `خرید تکی پوشاک زنانه مشهد | پوشاک ترنم`.

## 2026-09-05 — قالب قدیمی تلگرام دیگر پیام زنده را نمی‌برد

- سه ارسال تستی پریما هنوز `{name} — {price} + لینک` بود؛ در دیتابیس همان قالب قدیمی مانده بود و لایهٔ پارس آن را به‌عنوان متن زنده می‌گرفت.
- قالب قدیمی الان خودکار به شکل کانال ارتقا می‌یابد؛ پیش‌نمایش متن و تعداد عکس همان محصول را نشان می‌دهد.
- گزارش: `docs/reports/2026-09-05-omnichannel-admin-console.md`

## 2026-09-05 — کلمات کلیدی هفته (تکی و عمده)

- هوم تکی روی برند + خرید تکی ماند؛ لینک شومیز/کت/کاپشن/کفتان اضافه شد تا دسته مالک کوئری باشد نه هوم.
- دسته شومیز/کت/کفتان و متای کاپشن بامبری با متن انسانی بازنویسی شد؛ «خرید عمده» از SERP تکی حذف می‌شود.
- هوم/درباره/ثبت‌نام عمده روی تولیدی مشهد، حداقل سفارش ۶ عدد، و «همکاری با تولیدی لباس» تنظیم شد.
- گزارش: `docs/reports/2026-09-05-keyword-week-h2h.md`
...
- کاتالوگ ارسال نشد. سقف canary همچنان ۱۰ است. پیش‌نویس ادمین dry-run ماند؛ دکمهٔ «ارسال زنده به canary» جدا اضافه شد.
- گزارش: `docs/reports/2026-09-05-omnichannel-admin-console.md`

## 2026-09-05 — کنسول کانال‌های انتشار و رفع «؟؟؟»

- «؟؟؟» از خراب شدن فارسی در SSH ویندوز بود، نه از ربات. پیام اصلاح‌شدهٔ UTF-8 ارسال شد.
- پنل ادمین زبانه/آمادگی/برچسب گرفت؛ دکمهٔ «پیام فارسی» متن را از سرور می‌فرستد.
- گزارش: `docs/reports/2026-09-05-omnichannel-admin-console.md`

## 2026-09-05 — توکن ربات روی VPS و پیام آزمایشی canary

- توکن فقط در `.env` سرور ذخیره شد؛ در ادمین و گیت نیست.
- `getMe` موفق: ربات `TaranomRetailBot`. یک پیام آزمایشی به مقصد عددی canary رسید.
- اتصال «ربات تک» فعال شد. گزارش همان فایل فلگ زنده به‌روز شد.

## 2026-09-05 — فلگ زنده کانال‌های انتشار (دستور مالک)

- روی VPS هر دو فلگ روشن شد: `OMNICHANNEL_CONNECTORS_ENABLED` و `OMNICHANNEL_AUTO_PUBLISH`.
- API و هر دو ورکر recreate شدند؛ health ۲۰۰؛ صف ارسال تلگرام خالی ماند (۰).
- توکن ربات روی سرور نیست؛ پیام زنده ارسال نشد. گزارش: `docs/reports/2026-09-05-omnichannel-live-flags.md`

## 2026-09-05 — معماری موبایل فروشگاه تکی و توضیحات PDP

- شِل تکی (`retail-root`) overflow افقی را قطع می‌کند؛ هدر روی عرض کوچک با نام کوتاه «ترنم» جمع می‌شود.
- توضیحات محصول تکی مثل عمده زیر گالری/خرید می‌آید (دسکتاپ و موبایل)، نه داخل آکاردئون ستون خرید.
- بنر دسته روی موبایل تکی دیگر نوار ۲۱:۹ نیست. گزارش: `docs/reports/2026-09-05-retail-mobile-shell.md`

## 2026-09-04 — بنر دیجی‌پی هیرو تکی با متن جدا از تصویر

- اسلاید اول هیرو فروشگاه تکی: تصویر بدون نوشته؛ تیتر، توضیح و CTA در HTML/CMS.
- alt توصیفی برای عکس تنظیم شد. نسخه دسکتاپ حدود ۴۷KB و موبایل حدود ۳۷KB است.
- ادمین از `/admin/site-content` می‌تواند اسلاید، alt و تم روشن/تیره را عوض کند.
- migration فقط اسلاید دیجی‌پی را به هیرو منتشرشدهٔ تکی اضافه می‌کند و بقیه اسلایدها را نگه می‌دارد.
- درگاه و اعتبارنامه دیجی‌پی در این کار تغییر نکرد.

## 2026-09-04 — ارسال محصولات تکی به غرفه باسلام

- در تنظیمات ادمین دکمه «ارسال محصولات تکی به غرفه» اضافه شد؛ محصول جدید با API رسمی باسلام ساخته می‌شود و تکراری‌های غرفه فقط نگاشت می‌شوند.
- محصول تازه به‌صورت پیش‌نویس می‌ماند تا عکس در پنل باسلام بررسی و منتشر شود.
- دکمه جدا برای به‌روزرسانی موجودی و قیمت محصولات وصل‌شده.

## 2026-09-04 — حذف آدرس ارسال در پنل مشتری

- در پنل تکی (`/account/addresses`) و پروفایل عمده، هر آدرس ارسال دکمهٔ حذف دارد و قبل از پاک شدن تأیید می‌گیرد.
- آدرس‌های قدیمی بدون UUID دیگر با شناسهٔ تصادفی در هر بارگذاری گم نمی‌شوند؛ حذف با همان شناسهٔ پایدار کار می‌کند.
- آدرس فقط-محلی تکی (بدون شناسهٔ سرور) از لیست حساب و localStorage برداشته می‌شود.

## 2026-09-04 — کلیک‌های باقی‌ماندهٔ کنسول جستجو

- سایت‌مپ www و apex تکی و سایت‌مپ عمده همه Success هستند؛ هیچ‌کدام پاک نشد.
- بعد از تست زنده، ایندکس هوم تکی و هوم عمده یک‌بار درخواست شد.
- Missing image در Merchant الان Passed و صفر آیتم است؛ Validate Fix زده نشد.
- CWV موبایل عمده هنوز ۱۰ URL ضعیف با LCP بالای ۴ ثانیه است (آخرین آپدیت ۲ شهریور)؛ Validate Fix نزده شد.

## 2026-09-04 — جداسازی نشست پنل تکی و عمده

- ورود تکی JWT با `purpose=retail` می‌سازد؛ ورود پورتال `wholesale`. استراتژی دیگر همه را storefront نمی‌کند.
- روی دامنهٔ زنده `/portal` از `.ir` به `.com` و `/account` از `.com` به `.ir` می‌رود؛ لوکال دست نخورده است.
- شمارهٔ عمدهٔ PENDING می‌تواند با OTP وارد فروشگاه تکی شود بدون تأیید عمده. حساب فقط-تکی وارد پورتال نمی‌شود.
- کیف‌پول روی هر دو پنل ماند. اعلان عمده از منو برداشته شد تا API آماده شود.
- کوکی تکی و عمده جدا شد. `order.controller` در این برش لمس نشد.

## 2026-09-03 — سخت‌سازی حساب بعد از پنل مشتری

- بازبین پنل و امنیت هر دو PASS WITH CONDITIONS؛ must-fix نبود. زنده `5a23424`.
- بعد از عوض/تعیین/بازیابی رمز، JWTهای قدیمی با `passwordChangedAt` رد می‌شوند و نشست فعلی توکن تازه می‌گیرد.
- ریدایرکت ورود عمده/ادمین فقط مسیر همان پنل است. رقم فارسی در OTP تک قبول می‌شود. hint ثبت‌نام عمده ۸ کاراکتر است.
...
- لوگوی هدر عمده دیگر `<img>` نیست (پس‌زمینه CSS) تا React آن را جلوتر از هیرو preload نکند.
...
## 2026-09-03 — پنل مشتری و بازیابی رمز

- بازبین مستقل پنل و امنیت هر دو PASS WITH CONDITIONS؛ must-fix نبود. زنده `6dde78f`.
- فراموشی رمز عمده دیگر صفحهٔ تماس نیست: OTP جدا از ورود تکی، بدون افشای وجود شماره.
- مشتری واردشده می‌تواند رمز تعیین کند (بدون رمز فعلی) یا با رمز فعلی عوض کند.
- پنل تکی مسیرهای جدا برای سفارش، جزئیات، مشخصات، آدرس، امنیت، علاقه‌مندی و مرجوعی دارد.
- سایدبار عمده نام واقعی مشتری را نشان می‌دهد؛ اعلان ساختگی برداشته شد؛ PDF فاکتور با توکن دانلود می‌شود.
