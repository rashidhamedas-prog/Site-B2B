...
- پنل ترب‌پی فروشگاه ۰ سفارش موفق.
- فیکس کد: تست اتصال ادمین و شروع پرداخت روی ۱۱۰۰ fail واضح می‌دهد (دیگر soft-success نیست).
...
- `/retail/*`: بازنویسی روی `next.config` beforeFiles (بدون `x-middleware-rewrite`)؛ درخواست مستقیم `/retail` → ۳۰۱ به URL عمومی؛ `robots` Disallow `/retail`.
- کلکسیون خالی: اگر جدول Collection خالی باشد، دسته‌های فعال به‌عنوان کارت‌های کلکسیون نشان داده می‌شوند.
- PDP: کپی موجودی بدون «سایز سایز»؛ snap قیمت نزدیک هزارتومان؛ og:image روی مبدأ `.ir`؛ H1 برای صفحات CMS؛ چک‌اوت تا hydrate سبد صبر می‌کند.
- گزارش: `docs/reports/2026-09-25-retail-audit-fixes.md`.

## 2026-09-24 — اصلاح ابعاد و مقیاس چاپ برگه بسته A5 در پرینتر

- اصلاح استایل‌های پرینت در `packing-slip.css`:
  - تنظیم دقیق `@page { size: 148mm 210mm; margin: 0; }` جهت حذف هدر و فوتر پیش‌فرض مرورگر (URL و تاریخ) و تثبیت ابعاد واقعی کاغذ A5.
  - تنظیم ابعاد ثابت `width: 148mm` و `height: 208mm` روی `.ps-sheet` با پدینگ ایمن ۵mm در ۶mm جهت جلوگیری از خطای باگ چاپ ۲ صفحه‌ای کروم (sub-pixel rounding) و ممانعت از ایجاد صفحه سفید اضافه.
  - فشرده‌سازی و بازتنظیم متناسب فواصل عمودی (هدر، فرستنده، باکس گیرنده، جدول تطبیق و فوتر) برای جاگیری قطعی و پایدار سفارشات در ۱ برگ A5.
  - اصلاح چرخه حیات رویداد چاپ در `AdminPackingSlip.tsx`: حذف پاکسازی زودهنگام کلاس پرینت پس از ۱٫۵ ثانیه و اطمینان از اعمال کامل استایل حین تعامل کاربر با پنجره پیش‌نمایش چاپ.

## 2026-09-24 — بازطراحی مینیمال برگه بسته A5 (Swiss / Atelier)

- برگه بسته‌بندی و برچسب ارسال ادمین (`/admin/orders`) به سبک مینیمال با پس‌زمینه کاملاً سفید و خطوط مویی ۰٫۵ تا ۱٫۲۵ پوینت بازطراحی شد.
- پرکننده‌ها و نوارهای سنگین رنگی حذف شدند تا در چاپگرهای لیزری و حرارتی بدون پخش جوهر و با کنتراست حداکثری چاپ شود.
- سلسله‌مراتب وزیرمتن: نام گیرنده ۱۸px بولد، آدرس ۱۱٫۵px خوانا، کدپستی ۱۰ خانه‌ای با فاصله ۵+۵ و رقم ۱۴px در مرکز، جدول اقلام با کادر تطبیق خودکار، فوتر مبالغ و عبارت «تطبیق و بررسی شد».
- گزارش طراحی در `docs/reports/2026-09-13-packing-slip-design.md` به‌روزرسانی شد.

## 2026-09-23 — فروش داشبورد فقط بعد از ارسال

- آمار فروش ادمین (کارت ماه، نمودار، گزارش‌ها، شهر، پارچه، پرفروش‌ها، برترین مشتریان) فقط سفارش‌های `SHIPPED` / `DELIVERED` / `COMPLETED` است.
- بازهٔ زمانی با `shippedAt` است؛ اگر خالی باشد تاریخ ثبت سفارش. سفارش در صف پردازش داخل فروش نمی‌آید.
- شمارنده‌های عملیاتی (در انتظار، اخیر، تفکیک وضعیت) و مطالبات معوق عوض نشدند.

## 2026-09-23 — بازطراحی پنل همکار بازاریاب

- ناوبری پایین از هشت تب اسکرولی به چهار مقصد خانه، محصولات، سفارش‌ها و حساب جمع شد. پورسانت، تسویه، شبا و آموزش زیر حساب ماندند.
- متن دکمه خانه و حالت خالی کاتالوگ با مرور چت‌جی‌پی‌تی روی حساب واردشده تنظیم شد. درصد پورسانت و متن حقوقی عوض نشد.
- کتابخانه جدید اضافه نشد تا حجم صفحه بالا نرود.



## 2026-09-23 — لینک فروش اختصاصی همکار بازاریاب

- دیپلوی `56423fc` روی VPS. Health API 200. تنظیمات عمومی همچنان `enabled` و `termsVersion=2026-09-23-v1`. لینک نامعتبر `/go/sp/...` با ۳۰۲ به صفحه محصول می‌رود و کوکی نمی‌گذارد.
- هر همکار فعال `publicCode` دارد. لینک محصول `{retail}/go/sp/{code}/{slug}` است و فقط همان کالاهای کلیک‌شده، اگر در کاتالوگ مجاز باشند، روی سفارش خرده‌فروشی `RETAIL_WEBSITE` ثبت می‌شود.
- سفارش همچنان `affiliateId` نمی‌گیرد. خودارجاعی پورسانت ندارد. ارسال و کیف پول داخل پورسانت نیست. درصد برنامه اختراع نشده؛ تا ثبت نرخ در ادمین، تخمین صفر است.
- کاتالوگ همکار تا مجاز شدن محصول در `/admin/sales-partners` خالی می‌ماند.

## 2026-09-23 — LIVE همکار بازاریاب

- کلید اختصاصی `SALES_PARTNER_IBAN_KEY` روی API پرود تنظیم شد (مقدار در گیت نیست).
- شرایط همکاری نسخه `2026-09-23-v1` با تأیید مالک در `/sales-partnership/terms` منتشر شد.
- تنظیمات برنامه: `mode=LIVE`، ثبت‌نام باز، نگهداری پورسانت ۱۴ روز.
...
- فلگ‌های ریسک فقط برای ادمین محاسبه و نمایش داده می‌شوند؛ فرمول به همکار نشان داده نمی‌شود.
- بعد از ارسال لینک، تأیید یا رد مشتری، فلگ‌ها از روی پیش‌سفارش‌ها به‌روز می‌شوند. تعلیق خودکار نیست.
...
- تبدیل پیش‌سفارش این فیلدها را می‌نویسد و `affiliateId` خارجی را خالی می‌کند.
- ادمین فقط با دلیل و قبل از ثبت پورسانت می‌تواند attribution را عوض کند.

## 2026-09-23 — هشدار قیمت و موجودی پیش‌سفارش همکار

- پیش‌سفارش باز اگر قیمت یا موجودی فروشگاه عوض شده باشد، در خانه و فهرست سفارش هشدار انسانی می‌گیرد.
- سفارش تبدیل‌شده هشدار کهنه نمی‌گیرد؛ مبلغ نهایی همچنان هنگام تأیید از سرور محاسبه می‌شود.
- فرم سفارش جدید تا ارسال پیامک، مقادیر را روی همین دستگاه نگه می‌دارد و loading کاتالوگ را با «محصولی نیست» قاطی نمی‌کند.

## 2026-09-23 — وضعیت انسانی سفارش همکار بازاریاب

- بعد از تبدیل پیش‌سفارش، برچسب پنل همکار و فهرست ادمین از وضعیت سفارش تکی می‌آید (پرداخت، آماده‌سازی، ارسال، تحویل، لغو، مرجوعی).
- بعد از تحویل، اگر پورسانت هنوز در hold باشد «در انتظار آزادشدن پورسانت» و اگر آزاد شده باشد «پورسانت قابل‌برداشت» دیده می‌شود.
- تسویه‌ها loading را با empty قاطی نمی‌کنند و تاریخ را با `Asia/Tehran` نشان می‌دهند.

## 2026-09-22 — برنامه همکار بازاریاب (SalesPartner)

- نقش جدید از تأمین‌کننده ارسال (`Vendor` / `/partners`) و افیلیت خارجی جدا است.
- سند معماری: `docs/architecture/sales-partner-program.md`.
- مسیرها: `/sales-partnership`، `/sales-partners`، `/admin/sales-partners`.
- عنوان ادمین `/admin/partners` به «تأمین‌کننده ارسال» تغییر می‌کند تا با بازاریاب اشتباه نشود.
- فاز ۲: کاتالوگ مجاز، قوانین پورسانت، margin guard کالای تأمین‌کننده (بدون افشای رقم Vendor).
- فاز ۳: پیش‌سفارش، پیامک لینک یک‌بارمصرف، صفحه تأیید مشتری، تبدیل به `RETAIL_WEBSITE` با `idempotencyKey` و بدون `affiliateId`.
- فاز ۴–۶: دفتر نگهداری/برگشت، پنل همکار (واریانت، شبا masked، تسویه)، تسویه ادمین با idempotency.
- فلگ برنامه پیش‌فرض خاموش است. هنوز زنده نیست.
- Reviewer و Security مستقل فاز ۱–۶ را با شرط LIVE بررسی کردند. بعد از آن: برگشت پورسانت فقط اگر earned وجود داشته باشد؛ مرجوعی RMA فقط همان قلم؛ تخفیف سفارش در snapshot؛ شبا AES-256-GCM.
- فاز ۷ ادامه: GET جزئیات سفارش همکار، HTML پنل no-store، تست isolation برای IDOR/purpose/affiliate.
- ادمین: تنظیمات برنامه، فهرست سفارش همکاران، تعلیق حساب، گزارش و سوابق. رویداد outbox بدون داده حساس. ACL فقط ADMIN.

## 2026-09-22 — بازطراحی همکاران فروش در ادمین

- در `/admin/partners` می‌شود همکار را ویرایش کرد: نام، موبایل ورود، مهلت قبول سفارش، نگهداری تسویه، یادداشت و وضعیت.
- حذف فقط وقتی مجاز است که کالا، مرسوله، قلم سفارش و دفتر کمیسیون نداشته باشد. در این حالت حساب ورود هم پاک می‌شود تا شماره دوباره قابل دعوت باشد.
- همکار دارای سابقه حذف نمی‌شود؛ به‌جای آن تعلیق پیشنهاد می‌شود.
- فهرست جستجو و فیلتر وضعیت دارد و دفتر تسویه در پنل کناری باز می‌شود. ویترین و لندینگ دست نخورده است.
- دیپلوی کامل اجباری روی VPS `573d98a`؛ health ۲۰۰؛ هوم عمده ~۰٫۱۳s و تکی ~۰٫۱۸s. کلیک ادمین انجام نشد (ورود مدیر).
- گزارش: `docs/reports/2026-09-22-admin-partners.md`.

## 2026-09-22 — فیلتر پیشرفته فهرست محصولات ادمین

- در `/admin/products` می‌شود کاتالوگ را با دسته‌بندی (از جمله بدون دسته)، وضعیت، کالکشن و موجودی جدا کرد.
- فیلتر در آدرس صفحه می‌ماند و با کانال و جستجو جمع می‌شود. دستهٔ اصلی و عضویت اضافه هر دو شمرده می‌شوند.
- شناسه نامعتبر ۴۰۰ می‌دهد و داخل SQL قرار نمی‌گیرد. خروجی اکسل و فیلتر ویترین عوض نشد.
- زنده روی VPS `d9208b2`؛ health و هوم تکی/عمده ۲۰۰. کلیک ادمین انجام نشد (ورود مدیر).
- گزارش: `docs/reports/2026-09-22-admin-product-list-filter.md`.

## 2026-09-13 — workspace محتوای سایت ادمین هم‌گام شد

- `/admin/site-content` کانال و صفحه را در URL نگه می‌دارد؛ سئوی هر صفحه با بلوک‌ها ذخیره می‌شود.
- آلت جدا برای تصویر/گالری CMS؛ آلت گالری محصول همان کارت محصول است.
- صفحات تکی حریم خصوصی و قوانین اضافه شد و sitemap تکی آن‌ها را دارد.
- زنده روی VPS `b628674`؛ ۲۱ ردیف `site_contents` ذخیره شد (۹ صفحه تکی ساخته شد؛ SEO خالی پر شد؛ heroهای زنده عوض نشد).
- عنوان زنده: `.ir` «خرید تکی پوشاک زنانه مشهد»؛ `.com` «تولیدی مانتو مشهد | خرید عمده»؛ `/privacy` هر دو دامنه ۲۰۰ با description ذخیره‌شده.
- health ۲۰۰؛ هوم تکی ~۱٫۱۹s و عمده ~۱٫۰۳s.
- گزارش: `docs/reports/2026-09-13-admin-cms-workspace.md`.

## 2026-09-13 — workspace تنظیمات ادمین هم‌گام شد

- `/admin/settings` بخش‌ها را در URL نگه می‌دارد؛ هویت، ناوبری، ارسال، پیامک، پرداخت، اقساط، سئو، پیکسل و ظاهر کامل شدند.
- ذخیره هر بخش بعد از reload، ویترین تکی و عمده را تازه می‌کند؛ Organization و عنوان/OG از همان تنظیمات عمومی خوانده می‌شوند.
- آلت لوگو و OG در تنظیمات است؛ آلت گالری محصول همان `imageAlts` کارت محصول است.
- زنده روی VPS `696f6e8`؛ health و هوم تکی/عمده 200؛ `/settings/public` گروه `seo` دارد.
- گزارش: `docs/reports/2026-09-13-admin-settings-workspace.md`.

## 2026-09-13 — سفارش و مرجوعی به پرونده مشتری وصل شد

- جزئیات سفارش ادمین به `/admin/customers/:id?tab=orders` لینک دارد.
- `/admin/rma` نوع بازپرداخت و مبلغ اعتبار کیف را نشان می‌دهد و به مشتری/سفارش وصل است.
- تأیید مرجوعی کیف‌پول از دفتر `updateBalance` می‌گذرد، نه SQL خام.
- زنده روی VPS `2c02f4d`؛ health و هوم تکی/عمده 200.
- گزارش: `docs/reports/2026-09-13-admin-crm-order-rma.md`.

## 2026-09-13 — workspace مشتریان ادمین و دفتر کیف پول

- `/admin/customers` فیلتر کانال/وضعیت/سگمنت را در URL نگه می‌دارد؛ پرونده مشتری بخش هویت تا بازاریابی دارد.
- کیف پول دفتر ضمیمه‌ای است؛ موجودی همان `customers.balance` است و ادمین با دلیل ثبت می‌کند.
- تکی `/account/wallet` و عمده `/portal/dashboard/wallet` همان دفتر را می‌خوانند.
- گزارش: `docs/reports/2026-09-13-admin-customer-workspace.md`.

## 2026-09-13 — TASK-20260913-009 بسته شد روی master

- کد وبلاگ ادمین در `80889fb` روی `origin/master` است؛ VPS همان خط را در `718f0f8` دارد.
- Claims آزاد شد.

## 2026-09-13 — workspace محصولات ادمین و آلت تصویر هم‌گام شد

- `/admin/products` فیلتر کانال در URL دارد؛ ویرایشگر بخش‌های هویت تا رسانه را کامل می‌کند.
- هر عکس محصول `imageAlts` جدا دارد؛ PDP، کارت، OG و JSON-LD همان متن را می‌خوانند.
- گزارش: `docs/reports/2026-09-13-admin-product-workspace.md`.

## 2026-09-13 — workspace وبلاگ ادمین هم‌گام شد

- `/admin/blog` یک کانال و تب در URL دارد؛ همه بخش‌ها همان کانال را می‌خوانند.
- فرم‌های دسته، نویسنده، تنظیمات، رسانه، حذف و CTA با API موجود کامل شدند.
- گزارش: `docs/reports/2026-09-13-admin-blog-workspace.md`.

## 2026-09-13 — صف‌های وضعیت سفارش ادمین هم‌تراز شدند

- تب‌های `/admin/orders` شمارنده، توضیح صف، اکشن قانونی و هم‌خوانی با جزئیات/مشتری دارند.
- `COMPLETED` از `DELIVERED` قابل رسیدن است؛ پردازش دیگر به تأیید برنمی‌گردد.
- منبع مشترک: `packages/shared-types/src/order-status.ts`.
- گزارش: `docs/reports/2026-09-13-admin-order-queues.md`.

## 2026-09-13 — پرداخت درب منزل و تحویل در محل پیش‌فرض خاموش شدند

- نقدی عمده هم مثل تکی opt-in است؛ بدون سوییچ ادمین در چک‌اوت دیده نمی‌شود.
- «تحویل در محل» فقط با `inPersonEnabled` روشن می‌شود؛ لیست ذخیره‌شده به‌تنهایی آن را برنمی‌گرداند.
- گزارش همان `docs/reports/2026-09-13-cash-on-delivery-setting.md`.

## 2026-09-13 — پرداخت درب منزل فقط با سوییچ تنظیمات دیده می‌شود

- ریشه: کارت نقدی چک‌اوت سخت‌کد بود و API همیشه `CASH` را قبول می‌کرد؛ خاموش کردن ارسال حضوری یا درگاه دستی اثری نداشت.
- تکی و عمده حالا فلگ جدا در `/admin/settings` → درگاه پرداخت دارند. سرور سفارش نقدی خاموش را رد می‌کند.
- پیش‌فرض تکی خاموش است تا با قصد فعلی اپراتور یکی باشد. عمده نقدی مگر خاموش شود می‌ماند.
- گزارش: `docs/reports/2026-09-13-cash-on-delivery-setting.md`.

## 2026-09-13 — خطای ۱۰۱۱ ترب‌پی دیگر آدرس کامل را ناقص نشان نمی‌دهد

- سفارش زنده `ORD-2026-00037` آدرس کامل مشهد با پلاک و کدپستی ۱۰ رقمی داشت؛ ترب‌پی همان پیام «آدرس را کامل‌تر بنویسید» را می‌داد.
- ریشه: خط خیابان سه‌بار «پلاک ۱۳۷» ذخیره می‌شد و توکن CPG فیلدهایی می‌فرستاد که totweb نمی‌فرستد. نگاشت ۱۰۱۱ هم همیشه آدرس را مقصر می‌کرد.
- یک پلاک، سبد totweb، نام با هر دو کلید، و ذخیرهٔ آدرس تازه روی retry.
- زنده `216ff56`: همان سفارش بعد از آدرس تمیز و پیلود بدون آدرس هنوز ۱۰۱۱ است. واحد مبلغ ریال است (مثل totweb). باقیمانده سمت پذیرنده/ترب‌پی است.
- گزارش: `docs/reports/2026-09-13-torobpay-token-1011.md`.

## 2026-09-13 — تأیید زرین‌پال دیگر سفارش را در انتظار پرداخت نمی‌گذارد

- ریشه: بعد از OK زرین‌پال، کسر موجودی با `FOR UPDATE` روی JOIN آیتم‌ها کل تراکنش را rollback می‌کرد.
- قفل فقط روی ردیف سفارش است؛ آیتم جدا خوانده می‌شود. Verify تکراری دوباره وضعیت سفارش را اعمال می‌کند.
- گزارش: `docs/reports/2026-09-13-zarinpal-capture-lock.md`.

## 2026-09-12 — ثبت و بررسی سفارش فقط بعد از پرداخت نهایی

- سفارش آنلاین با مبلغ مثبت دیگر همان لحظه «در بررسی» نمی‌شود؛ وضعیت `AWAITING_PAYMENT` است تا درگاه PAID شود.
- بعد از پرداخت موفق وارد `PENDING_REVIEW` می‌شود و همان‌جا SMS ثبت برای مشتری می‌رود.
- نقدی/اقساط و سفارش رایگان مثل قبل همان موقع در بررسی می‌مانند. پنل مشتری دکمه تکمیل پرداخت دارد.
- زنده روی VPS `29d1e15`: health 200؛ سه سفارش پرداخت‌نشده از صف بررسی خارج شد.


## 2026-09-12 — پر شدن خودکار فیلدهای آدرس در تسویه

- آدرس ذخیره‌شده در پنل مشتری به‌صورت یک خط `street` می‌ماند؛ موقع تسویه همان خط به کوچه/پلاک/واحد شکسته می‌شود و هر فیلد جدا پر می‌شود.
- آدرس پیش‌فرض همین کار را موقع باز شدن تسویه می‌کند. فروشگاه تکی دفترچه را از `/auth/me/profile` می‌خواند، نه فقط localStorage.
- معماری: `docs/architecture/checkout-shipping-address.md`.

## 2026-09-12 — ذخیره آدرس پنل مشتری و فیلد پلاک

- ویرایش آدرس در حساب تکی/پورتال با خطای `property plaque should not exist` قطع می‌شد؛ فرم کوچه/پلاک/واحد را همراه پیش‌نویس می‌فرستاد و DTO آن‌ها را رد می‌کرد.
- حالا پلاک داخل همان خط `street` ذخیره می‌شود و API این فیلدهای فرم را ۴۰۰ نمی‌کند.

## 2026-09-12 — ذخیره نام فارسی دسته و نمایش پاییزی روی خانه

- ریشه: ایندکس یکتای `name` شامل دسته‌های حذف‌شده بود؛ گرید خانه لیست را برعکس می‌کرد و با سقف ۱۰، دستهٔ یازدهم (پاییزی) را می‌برید؛ برچسب ویترین فقط توکن فارسی را نشان می‌داد و ISR کاتالوگ بعد از ذخیره تازه نمی‌شد.
- یکتایی فقط برای ردیف‌های زنده؛ خطای تکراری ۴۰۰ فارسی؛ گرید تا ۱۶ کارت با ترتیب API (جدید اول)؛ برچسب = همان `name` ذخیره‌شده.
- معماری: `docs/architecture/category-identity-and-home-merch.md`.

## 2026-09-12 — فرم آدرس چک‌اوت مطابق ترب‌پی

- فرم تحویل تک و عمده ساختاری شد: استان، شهر از فهرست، خیابان، کوچه/پلاک/واحد، کدپستی ۱۰ رقمی.
- رقم فارسی/عربی کدپستی دیگر حذف نمی‌شود؛ برای ترب‌پی به ۱۰ رقم لاتین تبدیل می‌شود.
- عمده حالا `shippingAddress` را با سفارش می‌فرستد. معماری: `docs/architecture/checkout-shipping-address.md`.

## 2026-09-12 — ریل فیلتر گرافیکی کاتالوگ عمده و تکی

- فیلتر صفحه `/products` عمده از دکمه‌های متنی به ریل چسبان با سواچ رنگ، بافت پارچه، کارت سایز و سوئیچ «فقط موجود» تبدیل شد.
- همان شِل روی فروشگاه تکی اعمال شد؛ گرید select بالای صفحه برداشته شد.
- دسکتاپ فیلتر را همان لحظه اعمال می‌کند؛ موبایل پیش‌نویس می‌گیرد و با «نمایش نتایج» می‌بندد. چیپ‌های قابل‌حذف بالای گرید هستند.
- قرارداد API عوض نشد؛ URL فیلتر همچنان `noindex,follow` است.
- نام‌های پارچهٔ فرعی صفحه زیر «سایر پارچه‌ها» جمع می‌شوند تا ریل کوتاه بماند.

## 2026-09-12 — رقم فارسی، دسته فارسی، عضویت چنددسته

- آمار عمده/تکی رقم را موقع نمایش فارسی می‌کند؛ ذخیره CMS لاتین می‌ماند.
- ساخت دسته با نام فارسی و فاصله دیگر slug را رد نمی‌کند؛ ادمین فیلد انگلیسی جدا دارد.
- میانبر خانه از دسته‌های زنده می‌آید؛ گرید بنر تازه‌ها را با پین CMS قاطی می‌کند نه فیلتر خاموش.
- محصول می‌تواند چند دسته داشته باشد؛ SKU همچنان از دسته اصلی است.
- روی master با `4b78bcf` رفت؛ مهاجرت عضویت اعمال شد (۶۰ ردیف).

## 2026-09-12 — بنر کمپین فقط روی صفحه اصلی

- هیرو عمده/تکی دیگر بنر خانه را روی درباره، تماس، شرایط عمده و بقیه صفحات CMS تزریق نمی‌کند.
- محدوده از `pageKey` می‌آید؛ اگر مشخص نباشد کمپین خانه اعمال نمی‌شود.
- معماری دسته/رقم/بنر در `docs/architecture/catalog-locale-page-heroes.md`.

## 2026-09-12 — نوار آمار فشرده صفحه اول عمده

- آمار زیر بنر عمده از نوار بلند اعداد به یک ریل فشرده با کارت سفید و آیکون ایزومتریک تبدیل شد.
- اعداد همان محتوای CMS هستند؛ آیکون از برچسب حدس زده می‌شود و در ادمین قابل override است.
- ارتفاع تقریبی نصف `py-12/14` قبلی است؛ بدون JS کلاینت و بدون تصویر اضافه.

## 2026-09-10 — راهنمای صفحه محصول + کارت محصولات مرتبط عمده

- عنوان «لینک‌های مرتبط» به «راهنما» تغییر کرد و بخش بالای توضیحات محصول آمد.
- هر لینک یک ردیف کامل با عکس و توضیح کوتاه است؛ عکس و متن از ادمین قابل تنظیم است و اگر خالی بماند از صفحه هدف برداشته می‌شود.
- محصولات مرتبط عمده با کارت تصویری (مثل ویترین تکی، با قیمت همکاری) نشان داده می‌شوند.

## 2026-09-10 — بنر هیروی تکی دیجی‌پی و پریما/نگین با طرح حرفه‌ای

- دو اسلاید تکی از نو ساخته شد: عکس + تایپ Vazirmatn روی ۱۹۲۰×۵۶۰، نه upscale JPG قدیمی.
- روی بنر پریما دیگر «عمده» و دامنه `.com` نیست؛ لینک‌ها `/products` و `/category/women-coats`.
- روی `poshaktaranom.ir` زنده است: اسلاید ۱ دیجی‌پی، اسلاید ۲ پریما/نگین با دکمه «دیدن کت‌ها».

## 2026-09-09 — بنر هیروی عمده با طرح حرفه‌ای

- دو اسلاید همکاری و کت از نو ساخته شد: عکس پرتره + تایپ Vazirmatn روی ۱۹۲۰×۵۶۰، نه upscale JPG قدیمی.
- آدرس‌ها همان‌دامنه ماند: `/portal/register` و `/category/women-coats`.

## 2026-09-09 — ثبت پرداخت دفتر همکار در ادمین (فاز ۸)

- `GET/POST /v1/vendors/:id/ledger` و `/ledger/pay` فقط ادمین.
- ردیف‌های قابل‌برداشت (AVAILABLE یا HELD آزادشده با زمان) به PAID می‌روند؛ بدون ردیف PAYOUT جدا.
- در `/admin/partners` دکمه «دفتر» + «ثبت پرداخت قابل‌برداشت».

## 2026-09-09 — دفتر کمیسیون همکار (فاز ۷)

- بعد از «ثبت تحویل»، خالص کالا منهای کمیسیون ترنم در `vendor_ledger_entries` می‌نشیند.
- تا `settlementHoldDays` در hold است؛ بعد قابل برداشت نشان داده می‌شود. پرداخت ادمین بعداً.

## 2026-09-09 — انقضای SLA قبول مرسوله (فاز ۶)

- اگر همکار تا `acceptBy` قبول نکند (یا رد کند)، مرسوله به ترنم (OWN) برمی‌گردد.
- اگر مرسوله OWN از قبل باشد، اقلام ادغام می‌شوند. پیامک به ادمین کانال مربوط.
- Cron هر دقیقه؛ دکمه «رد» در پنل همکار.

## 2026-09-09 — ثبت ارسال مرسوله همکار + کد رهگیری (فاز ۵)

- همکار بعد از قبول، کد رهگیری می‌زند و مرسوله `SHIPPED` می‌شود.
- مشتری روی جزئیات سفارش، کد هر مرسوله را می‌بیند؛ پیامک ارسال همکار جداگانه است.
- ستون `fulfillment_orders.trackingCode` اضافه شد.

## 2026-09-09 — پیامک قبول مرسوله به همکار (فاز ۴)

- بعد از شکستن سفارش، برای هر مرسولهٔ همکار (`PENDING_ACCEPT`) رویداد outbox ساخته می‌شود.
- Worker شماره همکار را از `vendors` می‌خواند و SMS می‌فرستد؛ شماره در payload outbox نیست.
- سوئیچ و متن قالب در تنظیمات پیامک ادمین قابل ویرایش است. تلگرام همکار بعداً.

## 2026-09-09 — شکستن سفارش به مرسوله همکار (فاز ۳)

- بعد از تسویه موجودی، سفارش به مرسوله‌های بدون‌نام (`مرسوله ۱` …) شکسته می‌شود.
- همکار فقط مرسولهٔ خودش را می‌بیند و می‌تواند قبول کند؛ مشتری نام همکار و کمیسیون را نمی‌بیند.
- `shippingFee` مشتری روی سفارش والد می‌ماند؛ روی مرسوله همیشه صفر است (پست واقعی با همکار).
...
- روی محصول می‌توان همکار، کمیسیون ۰–۹۰٪ و برند عمومی گذاشت. کالای همکار فقط روی تکی است.
- API عمومی `vendorId` و کمیسیون را برنمی‌گرداند. JSON-LD اگر برند خالی باشد برند ترنم را جعل نمی‌کند؛ فروشنده همان ترنم است.
- مشتری روی PDP مبدأ ارسال را نمی‌بیند. هزینه پست واقعی با همکار است.

## 2026-09-09 — ذخیره CMS تکی واقعاً به DB نمی‌رسید

- در پروداکشن `RETAIL/home` از `2026-09-08T14:22Z` بی‌حرکت بود؛ عمده بعداً آپدیت شده بود.
- API دیگر بدون `channel` صریح به WHOLESALE سوییچ خاموش نمی‌کند.
- ادمین کانال را در URL نگه می‌دارد، بعد از PUT از DB می‌خواند و تأیید می‌کند، دکمه ذخیره کانال را نشان می‌دهد.

## 2026-09-09 — دعوت همکار فروش (فاز ۱)

- جدول `vendors`، نقش `VENDOR`، ورود جدا با `purpose=vendor`.
- ادمین از `/admin/partners` دعوت می‌کند؛ رمز موقت فقط یک‌بار برمی‌گردد.
...
- هندلر به `/admin/cms/revalidate` منتقل شد تا Next آن را سرو کند. SKU روی API از قبل لایو بود.

## 2026-09-08 — معماری بلوک محصولات برتر CMS

- بلوک «محصولات برتر» دیگر فقط متن و UUID خام نیست: منبع خودکار/دستی، جستجو و ترتیب، دسته، موجودی، دکمه مشاهده همه، و جعبه پنل عمده از ادمین کنترل می‌شود.
- `GET /v1/products?ids=` ترتیب چینش را حفظ می‌کند (حداکثر ۱۶ شناسه معتبر)، کانال و ACTIVE سرورساید اعمال می‌شود.
...
- Live روی VPS `8e57985` (health 200؛ هوم `.ir` و `.com` 200).
- گزارش: `docs/reports/2026-09-08-featured-products-cms.md`. ADR: `docs/adr/2026-09-08-featured-products-block.md`.

## 2026-09-08 — تحویل در محل در روش‌های ارسال

- روش داخلی `IN_PERSON` برای تکی و عمده؛ هزینه صفر؛ اگر در لیست ذخیره‌شده نباشد به تنظیمات و چک‌اوت اضافه می‌شود.
...
- زنده روی `origin/master` + VPS `e284ae9`؛ health ۲۰۰؛ متدهای ارسال تکی و عمده از API جدا برگشتند.
...
- انتخاب روش پرداخت از دکمهٔ کپسولی به کارت رادیویی مشترک (توکن سبز/طلایی) تبدیل شد.
- CTA مبلغ قابل‌پرداخت را نشان می‌دهد؛ روی موبایل نوار چسبان دارد و دکمه شناور عمده در `/checkout` مخفی است.
- کارت ترب‌پی فقط اگر `eligible` آن را برگرداند دیده می‌شود؛ چک آدرس ۱۰۱1 زنده (کدپستی ۱۰ رقم، خیابان حداقل ۸ نویسه) حفظ شد.
- گزارش: `docs/reports/2026-09-08-checkout-payment-ui.md`.
- زنده روی `origin/master` + VPS `3ad49d2`؛ health ۲۰۰.

## 2026-09-08 — خطای ترب‌پی can't create order (1011)

- صدور توکن دیگر سبد چندخطی + ارسال جدا + `commissionType` و دستهٔ فارسی نمی‌فرستد؛ یک خط برابر مبلغ قابل‌پرداخت، دسته `general`، شناسه بدون خط تیره.
- آدرس کوتاه با استان/شهر کامل می‌شود؛ چک‌اوت برای ترب‌پی خیابان حداقل ۸ نویسه می‌خواهد.
- سفارش ناموفق زنده: کالا ۱۲٫۷ میلیون + ارسال ۱٫۶ میلیون = ۱۴٫۳ میلیون ریال؛ OAuth سالم بود، ذخیرهٔ سفارش سمت ترب شکست می‌خورد.

## 2026-09-08 — رفع صدور توکن ترب‌پی

- اعتبارنامهٔ فعال‌سازی از پنل لاگین‌شدهٔ ترب در تنظیمات زنده جایگزین شد (نه گیت). OAuth از VPS توکن گرفت.
- مبلغ آیتم سبد به‌صورت قیمت واحد ریال ارسال می‌شود؛ کدپستی ۱۰ رقمی قبل از صدور توکن چک می‌شود؛ پیام خطای CPG با کد خطا به چک‌اوت می‌رسد.

## 2026-09-08 — ادمین: گزارش، CRM، پیامک، پست، نوار روان

- گزارش‌ها و آمار در برابر خطای SQL دیگر صفحه را نمی‌بندد؛ منوی تکراری آنالیتیکس حذف و به `/admin/reports` ریدایرکت شد.
- فیلتر عمده/تکی CRM روی `customers.type` (B2B در برابر B2C/RETAIL) است، نه `businessType`.
- پیامک ادمین فقط بعد از پرداخت قطعی؛ سبد رهاشده بعد از ۳۰ دقیقه؛ اتمام موجودی به شماره ادمین. رویدادها جدا برای تکی و عمده.
- محاسبه پست پیشتاز (Tapin + نرخ محلی) با سوئیچ فعال/غیرفعال در تنظیمات.
- ویرایش نوار روان هوم از بالای `/admin/settings`، جدا برای تکی و عمده.
- زنده روی `origin/master` و VPS: `2cdb951`. migration `CartSmsOps1757289600001` اجرا شد (`cart_signals`, `sms_event_log`). API health 200.

## 2026-09-07 — بنر باریک، نوار روان، CTA قوی‌تر

- هیروی عمده، تکی، بوتیک و دسته روی یک قاب مشترک `2:1` / `24:7` (حداکثر ۵۶۰px دسکتاپ) آمد؛ ۸۲–۹۲vh برداشته شد.
- نوار خبری لوپ مثل تلویزیون فقط روی هوم، CSS-only، دکمه توقف و احترام به reduced-motion.
- متن دکمه‌ها عمل‌محور شد. فایل عکس‌ها دوباره encode نشد.
- زنده روی `origin/master` و VPS: `eb4a433`. API health 200؛ هوم `.com` و `.ir` 200؛ هیرو زنده ۱۹۱۴×۵۵۸.
- گزارش: `docs/reports/2026-09-07-slim-hero-ticker-cta.md`.

## 2026-09-07 — درگاه ترب‌پی CPG مثل دیجی‌پی

- پنل ادمین `/admin/settings` چهار مقدار فعال‌سازی ترب‌پی (کد پذیرنده، کلید، نام کاربری، رمز) به‌همراه نمایش چک‌اوت و تست اتصال دارد.
- چک‌اوت تکی در صورت فعال و کامل بودن تنظیمات، گزینه «ترب‌پی (اقساطی)» را نشان می‌دهد. پیش‌فرض همچنان زرین‌پال است.
- آداپتر رسمی: OAuth + صدور توکن سبد + verify و settle در بازگشت. رمزها فقط در تنظیمات/env می‌مانند، نه در گیت.
- گزارش: `docs/reports/2026-09-07-torobpay-cpg-retail.md`.
