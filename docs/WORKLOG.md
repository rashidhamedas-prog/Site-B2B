...
## 2026-09-08 — ذخیره محتوا فوراً روی ویترین

- ذخیرهٔ CMS هوم را در API می‌نوشت ولی HTML هوم با ISR و SWR یک‌سالهٔ Cloudflare کهنه می‌ماند.
- بعد از ذخیره، مسیر هوم و تگ کش کاتالوگ invalidate می‌شود. شناسهٔ دستی اگر SKU باشد (مثل WINTER-WEAR00009) دیگر دور ریخته نمی‌شود.
- بازبینی مستقل مشخص کرد runtime ابتدا rebuild نشده بود، Cloudflare هنوز HTML عمده را cache می‌کرد و SKU نامعتبر به کل کاتالوگ fallback می‌شد. hotfix، HTML پویا را فقط در edge از cache خارج می‌کند، خطای revalidate را به اپراتور نشان می‌دهد و فیلتر نامعتبر/بی‌نتیجه را خالی برمی‌گرداند.
