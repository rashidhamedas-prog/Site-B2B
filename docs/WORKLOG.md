...
## 2026-07-28 — قانون پک: جمع موجودی عمده مضرب حداقل سفارش

### خلاصه
- `minOrderQty` = اندازه پک عملیاتی؛ جمع `wholesaleStock` واریانت‌ها باید مضرب آن باشد
- پک نامتقارن مجاز: موجودی هر رنگ می‌تواند نامساوی باشد (مثلاً ۷ رنگ / پک ۵ عددی)
- اعتبارسنجی API روی create/update/remove واریانت + تغییر MOQ
- `PATCH /products/:id/variants/stocks` برای ذخیره دسته‌ای (ثبت تدریجی بدون گیر کردن)
- سفارش‌ها assert پک را رد نمی‌کنند (MOQ روی qty سفارش باقی است)
- UI واریانت: بنر پک، ویرایش inline، پخش پک، تکمیل باقیمانده، ذخیره همه

---

## 2026-07-28 — رفع ۴۰۴ PDP تکی به‌خاطر double-encode اسلاگ

### خلاصه
- اسلاگ فارسی بعد از rewrite میدلور گاهی percent-encoded می‌ماند؛ `encodeURIComponent` دوباره آن را encode می‌کرد و API محصول را پیدا نمی‌کرد
- `decodeRouteParam` + استفاده در PDP تکی/عمده؛ انتخاب رنگ تکی بر اساس `retailStock`
- روی سرور: `freeThreshold` روی ۵۰٬۰۰۰٬۰۰۰ ریال (= ۵ میلیون تومان در UI) تنظیم شد
