...
## قالب شبیه کانال (پست ۴۶۶۷)

پیام زندهٔ اول ساده بود چون لایهٔ ارسال فقط متن داشت. پست کانال «مانتو جلوباز کتان مدل کیان» آلبوم چندعکس + کپشن با ایموجی، مشخصات، قیمت، سری، ضمانت و تماس است.

حالا قالب `v:1` بلوکی است (عکس / عنوان / مشخصه / ضمانت / متن). بدون قالب ذخیره‌شده، پیش‌فرض عمده همان اساس پست کانال است و تکی نسخهٔ کوتاه‌تر با لینک فروشگاه است. عکس فقط از هاست‌های ترنم با HTTPS می‌رود (`sendPhoto` یا `sendMediaGroup`). کلاژ سروری ساخته نمی‌شود؛ آلبوم ۵تایی تلگرام همان چیدمان ۲+۳ است.

ادمین: چینش بلوک با بالا/پایین و پیش‌نمایش تیره شبیه تلگرام. ذخیره قالب ارسال زنده نیست.

## قالب قدیمی روی ارسال زنده (پریما)

سه پیام تستی `coats00011` هنوز این شکل بود:

`کت شنل مدل پریما — 1/220/000 تومان` + لینک فروشگاه.

علت تأییدشده: ردیف `omnichannel_channel_templates` برای تکی هنوز body قدیمی `{name} — {price} تومان\\n{url}` بود. لایهٔ پارس هر متن غیرJSON را یک بلوک متن می‌گرفت؛ بنابراین «تنظیمات جدید» در UI اگر همان رشته را نگه می‌داشت، ارسال زنده همان دو خط را می‌ساخت و `photos=0` می‌ماند.

اصلاح: `isLegacyProductTemplate` این رشته و پیچیدن تک‌بلوکی‌اش را پیش‌فرض کانال می‌کند؛ `ensureProductTemplates` موقع status/preview/ارسال، تکی و عمده را در دیتابیس عوض می‌کند اگر هنوز قدیمی باشند. قالب ساخت‌یافتهٔ کاربر بازنویسی نمی‌شود. پیش‌نمایش ادمین متن و تعداد عکس همان محصول را نشان می‌دهد و شناسه می‌تواند لینک/کد/اسلاگ باشد.
