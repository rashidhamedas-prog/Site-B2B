...
export default async function BlogPreviewPage({ params, searchParams }: Props) {
  ...
  // Preview requires admin JWT in cookie via client — server uses optional bearer from query only for staff tools.
  // Without token, refuse to avoid indexing draft content.
  if (!token) notFound();
    ...
    return (
      <div className="min-h-screen bg-amber-50">
        ...
        <article className="mx-auto max-w-3xl px-4 py-10">
          <h1 className="mb-4 text-2xl font-extrabold">{post.title}</h1>
          <p className="mb-6 text-sm text-gray-600">{post.excerpt}</p>
