...
- Last verified: 2026-09-06 — **TASK-20260905-003** implementing always-visible write-only token cards (Telegram/Bale/Rubika) on `ai/TASK-20260905-003-omni-admin-console`. Vault AES-256-GCM already on master (`b8eefe0`). This-pass UI not live until merge/deploy.
- 2026-09-06 TASK-20260903-004 live `9cadd9d` is incomplete: `require('sharp')` still fails in `taranom_api`, and `WholesaleHeroCacheBust1757151000004` never ran because nest emit omitted the new migration JS. Dockerfile hotfix (tsc emit + fail-closed sharp) is in the isolated worktree and must ship before backfill/Cloudflare/GSC. Do not claim 28-day field CWV recovery.
...
- Last verified: 2026-09-06 — **TASK-20260905-003** publishing console v2 **LIVE** (`4b77fc4` in live `c7ff21c`): 5-step setup, destination verify, template options (album/single/text, HTML bold, buttons), automation rules (OFF/CANARY/LIVE, caps, quiet hours, withdraw). Health 200, verify route guarded, home TTFB unchanged. Automation mode is still OFF until the owner walks the wizard. VPS disk was 100 % → now 83 % (see handoff; ops cleanup recommended). TASK-20260826-001 still **NOT DONE**.
- Last verified: 2026-09-06 — **TASK-20260906-001** implementing on `ai/TASK-20260906-001-retail-boutique-theme`: second retail storefront skin (boutique). Default remains classic. Not live until merge/deploy + admin switch.
- Last verified: 2026-09-06 — **TASK-20260906-003** (renamed from 001 after ID collision with retail boutique-theme task) LIVE `07d25ff`: per-channel internal-link SEO (retail/wholesale) with admin picker, suggest/validate, PDP render + ItemList JSON-LD. Migration `ProductInternalLinks1757116800001` (id=33) ran; `product_internal_link` table + 5 indexes present; retail/wholesale 200. Reviewer still required after API change.
...
- Last verified: 2026-09-05 — **TASK-20260905-003** channel-style Telegram templates (album + structured caption) coded; first live canary was still plain text. TASK-20260826-001 still **NOT DONE**.
...
- Last verified: 2026-09-03 — **TASK-20260903-002** logo compress on `ai/TASK-20260903-002-logo-compress`: public `logo-128.png` 20KB→6.8KB, public `logo-512.png` 253KB→14KB. Live host copies + baked after this merge/deploy.
...
- Last verified: 2026-09-03 — **TASK-20260826-001** §9 leftovers + CMS channel on `ai/TASK-20260826-001-s9-settings`: public CMS requires RETAIL|WHOLESALE; leftover settings stored with `*Chosen` and stay inert. Connectors off. Canary send / live §9 apply / Security still **NOT DONE**.
- Last verified: 2026-09-01 — **TASK-20260826-001** Admin channel settings implementing on `ai/TASK-20260826-001-admin-channel-settings`: OOS radios + Telegram canary + secretRef. Connectors off. Soak/canary live send/§9/Security still **NOT DONE**.
